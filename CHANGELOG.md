...
- **BagsEnh now offers the warehouse as a shortcut to the AllEnh hub.** The hub
  knows nothing of our functions — we hand it the action the same way we hand it
  the download URL, through `AllEnh_Register`. An older hub simply ignores the
  field, and no hub at all changes nothing: `/be alts` is unaffected.
